    ...
    check_zones: function() {
        $.getJSON('/api/zones', function(json) {
            ...
            for (var i=0; i<zones.length; i++) {
                zone = zones[i];
                if (zone.status != 'ok') {
                    opsy.notification.add(zone.name+' Poller Failure', 'Datacenter '+zone.name+' is not responding!', 'danger', zone.name+'-offline');
                } else {
...
$(document).ready(function() {
    opsy.registerTask('zone-check', 5, opsy.check_zones, true);
    (function($) {
